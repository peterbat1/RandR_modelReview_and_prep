# A Shiny app to explore Restore and Renew GDM models
#
# Peter D. Wilson
...
# 26 May 2016
#
# 17 June 2018: Added progress notifications;
...
# 25 July 2020: Made into a fully standalone application by incorporating functions
# from the R-package RandR.modelReview
#
# 2022-11-10 to 2022-12-14: Extensive revision of code; added properly-working
# 3D surface views using a Bootstrap modal window (ie shinyBS::bsModel) combined
# with platly package; greatly streamlined computation of genetic difference
# surface and climate matching regions.
#
...
basePath <- "/home/peterw/Data_and_Projects/RBG Projects/Restore and Renew/RandR_webtool_dev/RandR_modelReview_app/www/"
...
sqlPath <- paste0(basePath, "resources/RandR_web_db.sqlite")
...
ui <- bootstrapPage(
  ...
  absolutePanel(id = "controls", top = 1, right = 5, width = "20%",
                ...
                absolutePanel(top = "360px", height = "25%", left = "1%", width = "95%",
                              hr(),
                              uiOutput("occRecords")
                ),
                absolutePanel(
                ...
                absolutePanel(top = "635px", height = "30%", left = "1%", width = "95%",
                              hr(),
                              ...
                              absolutePanel(left = "51%",  width = "49%",
  ...
  bsModal("show3dStuff", uiOutput("title3d"), "show3d", plotlyOutput("plotyplot", width = "100%"))
)

#"Genetic differentiation surface"
###############################################################################
                   ...
                   session)
{
  ...
  onWebtool <- reactive({
    tmp <- read.csv(paste0(basePath, "resources/onWebtool_lookup.csv"), stringsAsFactors = FALSE)
    row.names(tmp) <- tmp[, "taxon"]
    return(tmp)
  })
  
  oz_poly_sf <- reactive({
    return(sf::st_read(paste0(basePath, "resources/australia_polygon.shp")))
  })
    ...
    if (!is.null(click))
    {
      if (input$gdm_model != "No model selected")
      {
        ...
        if (nrow(sf::st_intersection(thisPt, bounds_poly())) > 0)
        {
          ...
          tic("Threshold prediction raster")
  ...
  filteredData <- reactive({
    ...
    cleanedTaxonName <- gsub("D$|DQ$|DQK2$|DQK3$|south|north|K2|K3", "", taxonName())
    
    # Grab sample data from db here so it is independently available to DArT display option
    sampleResults <- RSQLite::dbGetQuery(RandRdb, paste0('SELECT recordID, longitude, latitude, recordType, sentToDArT FROM samples WHERE "taxon" = "', cleanedTaxonName, '";'))
    
    if ("R&R Samples" %in% input$showCheckGrp)
    {
      tissue_ind <- which(sampleResults$recordType == "tissue")
      if (length(tissue_ind) > 0)
        tissue_pts <- cbind(sampleResults[tissue_ind, c("recordID", "longitude", "latitude")], ptType = rep(2, length(tissue_ind)))
    } else
    {
      tissue_pts <- NULL
    }
    ...
    {
      voucher_ind <- which(sampleResults$recordType == "voucher")
      
      if (length(voucher_ind) > 0)
      {
        voucher_pts <- cbind(sampleResults[voucher_ind, c("recordID", "longitude", "latitude")], ptType = rep(1, length(voucher_ind)))
      }
      else
      {
        voucher_pts <- NULL
    ...
    if ("DArT Samples" %in% input$showCheckGrp)
    {
      DArT_ind <- which(sampleResults$sentToDArT == "yes")
      if (length(DArT_ind) > 0)
      {
        DArT_pts <- cbind(sampleResults[DArT_ind, c("recordID", "longitude", "latitude")], ptType = rep(3, length(DArT_ind)))
      }
      else
      {
        DArT_pts <- NULL
    ...
    if ("ALA herbarium" %in% input$showCheckGrp)
    {
      herb_ind <- which(sampleResults$recordType == "herbarium")
      
      if (length(herb_ind) > 0)
      {
        ALA_herb_pts <- cbind(sampleResults[herb_ind, c("recordID", "longitude", "latitude")], ptType = rep(4, length(herb_ind)))
      }
    }
    else
    {
    ...
    if ("Survey records" %in% input$showCheckGrp)
    {
      survey_ind <- which(sampleResults$recordType == "nswSurvey")
      
      if (length(survey_ind) > 0)
      {
        survey_pts <- cbind(sampleResults[survey_ind, c("recordID", "longitude", "latitude")], ptType = rep(5, length(survey_ind)))
      }
    }
    else
    {
    ...
    tmp <- rbind(voucher_pts, tissue_pts, DArT_pts, ALA_herb_pts, survey_pts)
  ...
  ###############################################################################
  # Update map to display user-selected occurrence data
  observe({
    df <- filteredData()
  ...
  output$plotyplot <- renderPlotly({
    ...
    if (!is.null(click))
    {
      ...
      border_sf <- sf::st_cast(sf::st_crop(oz_poly_sf(), sf::st_bbox(bounds_poly())), "POLYGON", warn = FALSE)
      border_sf <- cbind(id = rep(1, nrow(border_sf)), border_sf)
      gdm_ras <- gdmSurface()
      gdm_ras_cropped <-raster::crop(gdm_ras, raster::extent(sf::st_bbox(bounds_poly())))
      gdm_ras_matrix <- t(as.matrix(gdm_ras_cropped))
      
      gdm_ras_proj <- gdm_ras_cropped
      gdm_ras_proj[which(!is.na(gdm_ras_proj[]))] <- 0
      ...
      p <- plotly::plot_ly(x = raster::yFromRow(gdm_ras_cropped, 1:nrow(gdm_ras_cropped)),
                           ...
                           colorscale = "Jet",
                           height = 700) %>%
        # add_sf(x = ~y, y = ~x, z = 0, data = border_sf,
        #        stroke = I("black"),
        #        #colorscale = c("lightskyblue", "lightskyblue"),
        #        #color = ~id,
        #        showscale = FALSE) %>%
        add_surface(x = raster::yFromRow(gdm_ras_cropped, 1:nrow(gdm_ras_cropped)),
                    y = raster::xFromCol(gdm_ras_cropped, 1:ncol(gdm_ras_cropped)),
                    z = t(as.matrix(gdm_ras_proj)),
                    colorscale = c("grey95", "lightskyblue"),
                    showscale = FALSE,
                    showlegend = FALSE,
                    hoverinfo = list(flags = "text"),
                    hovertext = "gdm domain") %>%
        add_markers(x = click$lat,
                    y = click$lng,
                    z = 0, 
                    showlegend = FALSE,
                    marker = list(symbol = "diamond", color = "blue"))
  ...
  ###############################################################################
  output$title3d <- renderUI({ paste("Genetic differentiation surface:", taxonName())})
