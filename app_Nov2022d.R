...
ui <- bootstrapPage(
  ...
  bsModal("show3dStuff", "Genetic diversity surface", "show3d", size = "large", plotlyOutput("plotyplot", width = "100%"))
)
                   ...
                   session)
{
  ...
  output$plotyplot <- renderPlotly({
    ...
    if (!is.null(click))
    {
      ...
      #cat(taxonName(), "   ", click$lng, "   ", click$lat, "\n")
      
      gdm_ras <- gdmSurface()
      gdm_ras_cropped <- raster::crop(gdm_ras, raster::extent(sf::st_bbox(bounds_poly())))
      gdm_ras_matrix <- as.matrix(gdm_ras_cropped)

      p <- plotly::plot_ly(x = raster::xFromCol(gdm_ras_cropped, 1:ncol(gdm_ras_cropped)),
                           y = raster::yFromRow(gdm_ras_cropped, 1:nrow(gdm_ras_cropped)),
                           z = gdm_ras_matrix,
                           type = "surface",
                           colorscale = "Jet"
                           )
